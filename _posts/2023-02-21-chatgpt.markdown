...
The key matrix represents the "features" of the input data. We can think of it as a set of "answers" to the questions posed by the quary vector. The value matrix represents the actual "content" of the input data.
...
In GPT-3 they use an attention method called Multi-Head Attention. This essentially means that the above process is just repeated lots of times in parallell with different
...
For example, assume we have k = 3 and the top 3 tokens are ["?":0.14], ["today":0.05] and ["sir":0.003]. We we would convert the probabiltities so they sum to one and then sample with probability from the top 3 words.
...
So, we have our fine-tuned ChatGPT (fine-tuned GPT-3). Now, we are going to give ChatGPT a prompt and the have it generat multiple responses. For example,
given a prompt $$p$$ we generate four responses $$a$$,$$b$$,$$c$$, and $$d$$. These four respones are then ranked by a human labeler from most preferred response
...
and better and are definetely here to stay. In fact, right now OpenAI are working on GPT-4 and it will probably be released in 2023!
...
<a href = "https://arxiv.org/pdf/1706.03762v5.pdf">Attention Is All You Need</a> (Officical GPT-3 paper)
